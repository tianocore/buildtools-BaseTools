...
gEccErrorMessage = {
    ...
    ERROR_GENERAL_CHECK_NO_TAB : "'TAB' is not allowed in source code, please replace them with spaces",
    ERROR_GENERAL_CHECK_INDENTATION : "Indentation is not followed coding style",
    ERROR_GENERAL_CHECK_LINE : "The width of each line is not followed coding style",
    ...
    ERROR_C_FUNCTION_LAYOUT_CHECK_FUNCTION_NAME : """Function name should be left justified and followed by the beginning of the parameter list, and the closing parenthesis is on its own line and also indented two spaces""",
    ...
    ERROR_INCLUDE_FILE_CHECK_DATA : "Include files should contain only public or only private data but not contain code or define data variables",
    ...
    ERROR_DECLARATION_DATA_TYPE_CHECK_ENUMERATED_TYPE : "Enumerated Type should have a 'typedef' and the name is capital",
    ERROR_DECLARATION_DATA_TYPE_CHECK_STRUCTURE_DECLARATION : "Structure Type should have a 'typedef' and the name is capital",
    ERROR_DECLARATION_DATA_TYPE_CHECK_UNION_TYPE : "Union Type should have a 'typedef' and the name is capital",
    ...
    ERROR_NAMING_CONVENTION_CHECK_PATH_NAME : """Path name is not followed by below rules: 1. First character should be upper case 2. Existing lower case in a word 3. No space existence""",
    ERROR_NAMING_CONVENTION_CHECK_VARIABLE_NAME : """Variable name is not followed by below rules: 1. First character should be upper case 2. Existing lower case in a word 3. No space existence""",
    ERROR_NAMING_CONVENTION_CHECK_FUNCTION_NAME : """Function name is not followed by below rules: 1. First character should be upper case 2. Existing lower case in a word 3. No space existence""",
    ERROR_NAMING_CONVENTION_CHECK_SINGLE_CHARACTER_VARIABLE : "There should be no use of short variable name with single character",
    
    ERROR_DOXYGEN_CHECK_ALL : "",
    ERROR_DOXYGEN_CHECK_FILE_HEADER : "The file headers should be followed Doxygen special documentation blocks in section 2.3.5",
    ERROR_DOXYGEN_CHECK_FUNCTION_HEADER : "The function headers should be followed Doxygen special documentation blocks in section 2.3.5",
    ...
    ERROR_META_DATA_FILE_CHECK_LIBRARY_INSTANCE_DEPENDENT : "A library instance should has been defined for all dependent library classes",
    ...
    ERROR_META_DATA_FILE_CHECK_BINARY_INF_IN_FDF : "An INF file is specified in the FDF file, but not in the DSC file, then the INF file must be for a Binary module only",
    ERROR_META_DATA_FILE_CHECK_PCD_DUPLICATE : "Duplicate PCDs are found in both FDF and DSC",
    ERROR_META_DATA_FILE_CHECK_PCD_FLASH : "PCD settings in the FDF file should only be related to flash",
    ERROR_META_DATA_FILE_CHECK_PCD_NO_USE : "There should be no PCDs which is used in INF files but not specified in DSC or FDF files",
